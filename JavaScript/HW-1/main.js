...
const book = 123.965;
const tea = 90.2345;
const totalPrice = chocolate + book + tea;
const sumFloat = Math.floor(chocolate) + Math.floor(book) + Math.floor(tea);
...
document.writeln(`<i>Максимальне число: ${Math.max(chocolate, book, tea)} <br>
                Мінімальне число: ${Math.min(chocolate, book, tea)} <br> Вартість всіх товарів: ${totalPrice} <br> 
